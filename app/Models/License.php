    ...
    public function scopeByFilter($query, $filter)
    {
        return $query->where(function ($query) use ($filter) {
            foreach ($filter as $fieldname => $search_val) {
                ...
                elseif ($fieldname == 'depreciation') {
                    $query->whereHas('depreciation', function ($query) use ($search_val) {
                        $query->where('depreciations.name', 'LIKE', '%' . $search_val . '%');
                    });
                }
                elseif ($fieldname == 'supplier') {
                ...
                $query->where('licenses.'.$fieldname, 'LIKE', '%' . $search_val . '%');
                }
            }
        });
    }
